...
const BottomUpdateOrder: React.FC<Props> = ({ show, onDismiss, enableBackDropDismiss = true, ProductId }) => {
    ...
    return (
        <Modal animationType="slide" transparent={true} onRequestClose={onDismiss}>
            <Pressable onPress={enableBackDropDismiss ? onDismiss : undefined} style={StyleBottomUpdateOrder.backdrop} />
            <StatusBar backgroundColor="rgba(0,0,0,0.5)" />
            <Animated.View style={[StyleBottomUpdateOrder.container, { bottom: bottomsheet }]}>
                <PanGestureHandler onGestureEvent={onGestureEvent} onEnded={onGestureEnd}>
                    <View style={StyleBottomUpdateOrder.header}>
                        <Text style={StyleBottomUpdateOrder.texttitle}>{ProductId.ProductId.name}</Text>
                        <TouchableOpacity onPress={onDismiss}>
                            <Image source={Icon.CANCEL} style={StyleBottomUpdateOrder.iconcancel} />
                        </TouchableOpacity>
                    </View>
                </PanGestureHandler>
                                    ...
                                    <TouchableOpacity key={index} style={StyleBottomUpdateOrder.viewsizearray} onPress={() => setSize(sizeItem)}>
                                        <View style={StyleBottomUpdateOrder.viewcheckitem}>
                                            ...
                                            <View style={StyleBottomUpdateOrder.viewsizename}>
                                                ...
                                                <Text style={StyleBottomUpdateOrder.textsizeprice}>{FormatPrice(parseInt(sizeItem.price))}</Text>
                                            </View>
                                        </View>
                                        {ProductId.ProductId.size.length - 1 === index ? null : <View style={StyleBottomUpdateOrder.lineitem} />}
                                    </TouchableOpacity>
                                ))}
                ...
                <View style={StyleBottomUpdateOrder.viewbutton}>
                    <View style={StyleBottomUpdateOrder.viewquantity}>
                        <TouchableOpacity style={StyleBottomUpdateOrder.viewminus} onPress={handleMinus(quantity, setQuantity)}>
                            <Image source={Icon.MINUS} style={StyleBottomUpdateOrder.iconminus} />
                        </TouchableOpacity>
                        <Text style={StyleBottomUpdateOrder.textnumber}>{quantity}</Text>
                        <TouchableOpacity style={StyleBottomUpdateOrder.viewplus} onPress={handlePlus(quantity, setQuantity)}>
