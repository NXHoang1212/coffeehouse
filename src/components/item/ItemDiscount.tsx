...
const ItemDiscount: ({ item }: PropsItemDisount) => JSX.Element = ({ item }) => {
    ...
    return (
        <TouchableOpacity style={StyleItemDiscount.container} onPress={() => { dispatch(setDiscount(item)), setshow(true) }}>
            <View>
                <Image source={Icon.BACKGROUNDPROMO} style={StyleItemDiscount.imagebackground} />
            </View>
            ...
            <BottomSheetDiscount show={show} onDismiss={() => setshow(false)} Messenger={Messenger} />
        </TouchableOpacity>
    )
}
