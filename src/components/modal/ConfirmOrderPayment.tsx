...
const ConfirmOrderPayment: React.FC<
  ModalConfirmPaymentProps & {address: Address}
> = ({visible, onDismiss, enableBackDropDismiss, address, handlePayment}) => {
  const time = GetDelivery();
  const DescribeAddRess = address.DescribeAddRess.slice(
    0,
    address.DescribeAddRess.lastIndexOf(','),
  );
      ...
      <Pressable
        ...
        <View style={StyleConfirmOrderPayment.modalContainer}>
          <View style={StyleConfirmOrderPayment.body}>
            <Text style={StyleConfirmOrderPayment.texttitle}>
              Xác nhận thông tin đơn hàng
            </Text>
            <Text style={StyleConfirmOrderPayment.textbody}>
              Đơn hàng sẽ được giao tới chỗ bạn vào lúc {time} hôm nay tại địa
              điểm {DescribeAddRess}
            </Text>
          </View>
          <View style={StyleConfirmOrderPayment.viewbutton}>
            <TouchableOpacity onPress={onDismiss}>
              <Text style={StyleConfirmOrderPayment.textindex}>
                Thay đổi thông tin
              </Text>
            </TouchableOpacity>
