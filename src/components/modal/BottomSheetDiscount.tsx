import { View, Text, Animated, Image, TouchableOpacity, Pressable, ScrollView, Dimensions, StatusBar, Modal } from 'react-native';
...
const BottomSheetDiscount: React.JSXElementConstructor<Props> = ({ show, onDismiss, enableBackDropDismiss = true, Messenger }) => {
    ...
    const Quantity = data?.data?.map((item: CartOrder) => item.ProductId.map((item: any) => item.QuantityProduct)).flat().reduce((a: any, b: any) => a + b, 0);
    const total = data?.data?.map((item: CartOrder) => item.ProductId.map((item: any) => item.PriceProduct)).flat().reduce((a: any, b: any) => a + b, 0);
    ...
    const ApplyDiscount = () => {
        if (ProductId?.length === 0) {
            ...
            if (discount.description.includes('Miễn phí vận chuyển')) {
            ...
            } else if (discount.description.includes('Giảm 40% đơn từ 4 ly nước kèm FREESHIP')) {
                if (Quantity >= 4) {
            ...
            } else if (discount.description.includes('Giảm 50% đơn từ 10 ly nước kèm FREESHIP')) {
                if (Quantity >= 10) {
                ...
                if (total >= 99) {
                    ...
                    Messenger('Đơn hàng của bạn chưa đủ giá trị vui lòng mua thêm', 'error');
                }
            ...
            } else if (discount.description.includes('Giảm 10% cho đơn hàng từ 2 món')) {
                if (Quantity >= 2) {
                    const discountAmount: number = 0.1;
                    dispatch(setPromodiscount(discountAmount));
                    Messenger('Áp dụng mã giảm giá thành công', 'success');
                } else {
                    Messenger('Bạn còn thiếu một món vui lòng mua thêm để được giảm', 'error');
                }
            }
        } else {
                ...
                <PanGestureHandler onGestureEvent={onGestureEvent} onEnded={onGestureEnd}>
                    <View style={StyleBottomSheetDiscount.header}>
                        <TouchableOpacity onPress={onDismiss}>
                            <Image source={Icon.CANCEL} style={StyleBottomSheetDiscount.iconcancel} />
                        </TouchableOpacity>
                    </View>
                </PanGestureHandler>
                    ...
                    <View style={StyleBottomSheetDiscount.body}>
                        <View style={{ left: 2 }}>
                            <View style={StyleBottomSheetDiscount.viewtext}>
                                <Text style={StyleBottomSheetDiscount.textTitle}>{discount.Tilte}</Text>
                                <Text style={StyleBottomSheetDiscount.textName}>{discount.name}</Text>
                            </View>
                            ...
                            <View style={StyleBottomSheetDiscount.viewcoppy}>
                                ...
                                <Text onPress={onCoppy} style={StyleBottomSheetDiscount.textcoppy}>Sao chép</Text>
                            </View>
                            <TouchableOpacity style={StyleBottomSheetDiscount.viewusedto} onPress={ApplyDiscount}>
                                <Text style={StyleBottomSheetDiscount.textusedto}>Sử dụng ngay</Text>
                            </TouchableOpacity>
                            <View style={StyleBottomSheetDiscount.line} />
                            <View style={StyleBottomSheetDiscount.viewdateend}>
                                <Text style={StyleBottomSheetDiscount.textend}>Ngày hết hạn:</Text>
                                <Text style={StyleBottomSheetDiscount.textqr}>{discount.end}</Text>
                            </View>
                            <View style={StyleBottomSheetDiscount.line} />
                            <Text style={StyleBottomSheetDiscount.textdescription}>{discount.description}</Text>
                        </View>
                    </View>
                </ScrollView>
            </Animated.View>
