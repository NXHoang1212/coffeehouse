...
export const useAuth = () => {
  ...
  const login = async () => {
    try {
      ...
      console.log(
        '🚀 ~ file: UseAuth.tsx:12 ~ login ~ Đăng nhập thành công và đã lưu trạng thái đăng nhập vào AsyncStorage',
      );
      setLoggedIn(true);
    } catch (error) {
  ...
  const logout = async () => {
    try {
      ...
      console.log(
        '🚀 ~ file: UseAuth.tsx:22 ~ logout ~ Đăng xuất thành công và đã xóa trạng thái đăng nhập khỏi AsyncStorage',
      );
      setLoggedIn(false);
    } catch (error) {
